 ...
 *  - 16 region pages x 2 tracks (유아 한글떼기/초등 국영수) = 32 region pages
...
const SITE = 'https://startse.co.kr';
...
// Real facts from index.html/curriculum.html - 유아 한글떼기 4단계, 초등
// 국어·수학·영어 기초. Only region/district name varies per page.
const TRACKS = {
  hangul: { label: '유아한글과외', title: '유아 한글떼기', body: '자음·모음 인지부터 문장 읽기까지, 4단계로 나눠 놀이처럼 즐겁게 진행합니다.', tags: ['한글떼기 4단계', '자음모음 인지', '문장 읽기'] },
  elem: { label: '초등과외', title: '초등 국어·수학·영어', body: '학교 진도에 맞춰 국어 어휘력·독해력, 수학 연산, 영어 파닉스 기초를 다집니다.', tags: ['초등국어', '초등수학', '초등영어'] },
};
...
<link href="https://fonts.googleapis.com/css2?family=Jua&family=Pretendard:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<meta property="og:type" content="website">
<meta property="og:site_name" content="한글완전정복">
...
function header() {
  return `<header>
  <div class="wrap nav">
    <a class="logo" href="/"><span class="logo-mark">한</span>한글완전정복</a>
...
function footer() {
  return `<footer>
  <div class="wrap">
    <p>© 2026 한글완전정복. All rights reserved. · <a href="/" style="color:#fff;">홈으로</a></p>
  </div>
</footer>`;
}
function faqMini() {
  return `<section class="section-leaf">
...
function regionPageTemplate(region, track) {
  const title = `${region.name} ${track.label} | 한글완전정복`;
  const desc = `${region.name} 지역 ${track.title} 안내. ${track.body} 화상 수업이 기본이며, 지역에 따라 방문 수업도 상담 후 진행합니다.`;
  const canonical = `${SITE}/areas/${encodeURIComponent(regionSlug(region, track))}.html`;
  const keywords = `${region.name}${track.label}, ${region.name} ${track.label}, ${track.label}, ${region.name} 한글과외, ${region.name} 초등과외`;
...
<body>
${header()}
<div class="breadcrumb"><a href="/">홈</a><span class="sep">/</span><a href="/areas/">지역별 과외</a><span class="sep">/</span><span>${esc(region.name)} ${esc(track.label)}</span></div>
<div class="subpage-hero">
  <div class="wrap">
    <h1>${esc(region.name)} ${esc(track.label)}</h1>
    <p>한글완전정복이 ${esc(region.name)} 지역 ${esc(track.title)}를 안내합니다.</p>
  </div>
</div>
<section>
  <div class="wrap">
    <div class="point-grid">
      <div class="point-card"><h3>${esc(track.title)}</h3><p>${esc(track.body)}</p></div>
      ${track.tags.map(t => `<div class="point-card"><h3>${esc(t)}</h3><p>${esc(region.name)} 지역에서도 동일한 커리큘럼으로 진행합니다.</p></div>`).join('\n      ')}
    </div>
  </div>
</section>
${faqMini()}
...
<section class="cta-section">
  <div class="wrap" style="max-width:520px;">
    <h2>${esc(region.name)} ${esc(track.label)}, 무료 진단 받아보세요</h2>
    ...
    <p style="margin-top:16px; font-size:13px;">${esc(region.name)}의 다른 과정: ${otherTrack}</p>
    <p style="margin-top:8px;"><a href="/areas/" style="color:var(--leaf-dark); font-weight:700;">전국 지역별 과외 전체 보기 →</a></p>
  </div>
</section>
${footer()}
...
function districtPageTemplate(region, district, track) {
  ...
  const title = `${region.name} ${district_} ${track.label} | 한글완전정복`;
  const desc = `${region.name} ${district_} 지역 ${track.title} 안내. ${track.body} 화상 수업이 기본이며, 지역에 따라 방문 수업도 상담 후 진행합니다.`;
  ...
  const keywords = `${district_}${track.label}, ${district_} ${track.label}, ${region.name}${district_}과외, ${track.label}`;
...
<body>
${header()}
<div class="breadcrumb"><a href="/">홈</a><span class="sep">/</span><a href="/areas/">지역별 과외</a><span class="sep">/</span><a href="${parentUrl}">${esc(region.name)} ${esc(track.label)}</a><span class="sep">/</span><span>${esc(district_)}</span></div>
<div class="subpage-hero">
  <div class="wrap">
    <h1>${esc(district_)} ${esc(track.label)}</h1>
    <p>한글완전정복이 ${esc(district_)} 지역 ${esc(track.title)}를 안내합니다.</p>
  </div>
</div>
<section>
  <div class="wrap">
    <div class="point-grid">
      <div class="point-card"><h3>${esc(track.title)}</h3><p>${esc(track.body)}</p></div>
      ${track.tags.map(t => `<div class="point-card"><h3>${esc(t)}</h3><p>${esc(district_)} 지역에서도 동일한 커리큘럼으로 진행합니다.</p></div>`).join('\n      ')}
    </div>
...
<section class="cta-section">
  <div class="wrap" style="max-width:520px;">
    <h2>${esc(district_)} ${esc(track.label)}, 무료 진단 받아보세요</h2>
    <p>상담과 학습 진단은 무료이며, 등록을 강요하지 않습니다.</p>
    <div class="hero-actions" style="justify-content:center;">
      <a class="btn-primary" href="/#apply">무료 학습 진단 신청</a>
      <a class="btn-ghost" href="tel:010-3951-0535">☎ 전화 상담</a>
    </div>
    <p style="margin-top:8px;"><a href="${parentUrl}" style="color:var(--leaf-dark); font-weight:700;">${esc(region.name)} 전체 보기 →</a> · <a href="/areas/" style="color:var(--leaf-dark); font-weight:700;">전국 지역별 과외 전체 보기 →</a></p>
  </div>
</section>
${footer()}
<script src="/assets/site.js"></script>
...
<html lang="ko">
<head>
${head('전국 지역별 과외 전체 목록 | 한글완전정복', '전국 16개 광역지자체, 시/군/구, 유아 한글떼기·초등과외 안내 페이지 모음입니다.', `${SITE}/areas/`, '전국 한글과외, 지역별 유아과외, 지역별 초등과외')}
</head>
<body>
${header()}
...
<div class="subpage-hero">
  <div class="wrap">
    ...
    <p>전국 16개 광역지자체, 시/군/구, 유아 한글떼기·초등과외 안내 페이지를 정리했습니다.</p>
  </div>
</div>
<section>
  <div class="wrap">
